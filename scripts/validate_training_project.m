...
for index = 1:numel(modelNames)
    ...
    acceptedLicenseFallback = contains(modelWarnings(index), ...
        'using Accelerator mode are not supported with this license') && ...
        contains(modelWarnings(index), 'will run in Normal mode');
    assert(strlength(modelWarnings(index)) == 0 || acceptedLicenseFallback, ...
...
for index = 1:numel(modelNames)
    ...
    if strlength(modelWarnings(index)) > 0
        fprintf(fileId,'; accepted license fallback warning: %s', ...
            modelWarnings(index));
    end
    fprintf(fileId,'\n');
end
